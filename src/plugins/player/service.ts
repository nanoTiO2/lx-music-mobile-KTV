...
import { isTempId, isEmpty, shouldIgnoreTrackPlayerStateEvents, suppressTrackPlayerStateEvents } from './utils'
...
let lastEndedEmitAt = 0

const emitPlayerEnded = () => {
  const now = Date.now()
  if (now - lastEndedEmitAt < 1500) return
  lastEndedEmitAt = now
