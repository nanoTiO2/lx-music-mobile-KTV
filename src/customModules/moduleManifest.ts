...
export const customModuleManifest: CustomModuleDefinition[] = [
  ...
  {
    id: 'prompt_system_foundation',
    name: '舞台提词系统骨架模块',
    summary: '新增独立提词共享协议、LRC/SRT/ASS/TXT 文档解析、顺序点按与 JK 接力状态机，以及展示端 PromptStage 入口，为后续控制端与三链路通信打底。',
    platform: ['android', 'mobile-js', 'desktop-portable'],
    files: [
    ...
    upstreamMergeRisk: 'medium',
    desktopPortNotes: '共享协议、文档解析和状态机都可直接迁移到控制端独立仓或桌面端，当前页面主要复用为展示端原型。',
  },
  {
    id: 'prompt_control_loopback',
    name: '舞台提词控制端本地联调模块',
    summary: '在当前仓新增 PromptControl 控制页、本地 loopback 会话层、设置页被控入口与独立 PromptReceiver，被控端、控制端与展示端共享同一份提词状态。',
    platform: ['android', 'mobile-js', 'desktop-portable'],
    files: [
    ...
    upstreamMergeRisk: 'medium',
    desktopPortNotes: '本地 loopback 会话层可直接替换为桌面端窗口间同步或局域网链路；控制页、设置页被控入口和独立接收屏都可直接迁移。',
  },
]
