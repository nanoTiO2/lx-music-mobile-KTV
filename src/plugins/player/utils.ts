...
const ensureMixerForLocalPlayback = async() => {
  ...
  await startMixerTransition({
    ...
    fadeDurationMs: 60,
    volume: settingState.setting['player.volume'],
    fromGain: getMusicKtvVariantGain(musicInfo),
    toGain: getMusicKtvVariantGain(musicInfo),
  })
  await setMixerPlaybackRate(settingState.setting['player.playbackRate'])
  await setMixerPitch(semitonesToPitch(settingState.setting['player.pitchSemitones']))
  await applyMixerTrackGain(musicInfo)
...
const sanitizeMixerOutputVolume = (num: number) => {
  if (num < 0) return 0
  if (num > 3.2) return 3.2
  return num
}
...
const applyMixerTrackGain = async(
  musicInfo: LX.Player.PlayMusic | null = playerState.playMusicInfo.musicInfo,
  ktvGainOverride?: number,
) => {
  const activeGain = getMusicKtvVariantGain(musicInfo, ktvGainOverride)
  await setMixerTrackGains(activeGain, activeGain)
  return activeGain
}
...
const applyTrackOutputVolume = async(
  ...
  if (isMixerActive()) {
    const activeGain = await applyMixerTrackGain(musicInfo, ktvGainOverride)
    const mixedVolume = sanitizeMixerOutputVolume(sanitized * activeGain)
    await setMixerOutputVolume(mixedVolume)
    return TrackPlayer.setVolume(sanitizeOutputVolume(mixedVolume))
  }
...
export const setKtvVariantGain = async(gain: number) => {
  const nextGain = Math.max(0.6, Math.min(3.2, gain))
  const currentMusicInfo = playerState.playMusicInfo.musicInfo
  if (isMixerActive()) {
    const activeGain = await applyMixerTrackGain(currentMusicInfo, nextGain)
    await setMixerOutputVolume(sanitizeMixerOutputVolume(settingState.setting['player.volume'] * activeGain))
  }
